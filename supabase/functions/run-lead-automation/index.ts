...
interface ApifyPlace {
  ...
  website?: string | null
  phone?: string | null
  // Email fields — Apify may return either or both:
  emails?: string[]              // array of email strings
  contactEmail?: string | null   // single email field
  // Location fields:
  address?: string
  street?: string
  city?: string
  state?: string                 // Apify returns state separately
  country?: string
  postalCode?: string            // useful for future filtering
  // Business info:
  totalScore?: number
  reviewsCount?: number
  categoryName?: string          // the Google Maps category
  categories?: string[]          // all categories
  // Coordinates:
  location?: { lat: number; lng: number }
}

// Helper: extract email from Apify place data
function extractEmail(place: ApifyPlace): string | null {
  // Try emails array first (most reliable)
  if (place.emails && place.emails.length > 0) {
    const first = place.emails[0]
    // Basic validation — must contain @ and a dot after @
    if (first && first.includes('@') && first.split('@')[1]?.includes('.')) {
      return first.trim().toLowerCase()
    }
  }
  // Fall back to contactEmail
  if (place.contactEmail && place.contactEmail.includes('@')) {
    return place.contactEmail.trim().toLowerCase()
  }
  return null
}

// Helper: derive industry string from Apify data
function deriveIndustry(place: ApifyPlace, filterCategory?: string | null): string | null {
  // Prefer the automation's explicit category filter if set
  if (filterCategory && filterCategory.trim()) {
    return filterCategory.trim()
  }
  // Fall back to the place's primary Google Maps category
  if (place.categoryName && place.categoryName.trim()) {
    return place.categoryName.trim()
  }
  // Try first item from categories array
  if (place.categories && place.categories.length > 0) {
    return place.categories[0].trim()
  }
  return null
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 2. triggered_by (manual run: person who clicked Run Now)
    // 3. automation.created_by (scheduled run: use automation creator)
    const zeroUUID = '00000000-0000-0000-0000-000000000000'
    const resolvedOwnerId = automation.assign_to_user_id
      ?? (triggered_by === zeroUUID ? automation.created_by : triggered_by)
      ?? null

    const resolvedCreatedBy = triggered_by !== zeroUUID
      ? triggered_by
    ...
    for (const place of filtered) {
      ...
      // Build description from Google Maps data
      const description = [
        place.categoryName ? `Category: ${place.categoryName}` : null,
        place.totalScore ? `Rating: ${place.totalScore}/5 (${place.reviewsCount ?? 0} reviews)` : null,
      ...
      const leadData = {
        ...
        // Source tracking — ALL automation-created leads get these
        source: 'lead_generator',
        source_automation_id: automation.id,
        source_run_id: runId,
        source_automation_name: automation.name,  // "Created By" display field
        ...
        industry: deriveIndustry(place, automation.filter_category),
